...
" 2017 February 11: Modified as a syntax file for .lag files 
"                      (literate UUAG files)
...
if !exists("b:lag_markup")
    if exists("lag_markup")
	if lag_markup =~ '\<\%(tex\|none\)\>'
	    let b:lag_markup = matchstr(lag_markup,'\<\%(tex\|none\)\>')
	else
	    echohl WarningMsg | echo "Unknown value of lag_markup" | echohl None
	    let b:lag_markup = "unknown"
	endif
    else
	let b:lag_markup = "unknown"
    endif
else
    if b:lag_markup !~ '\<\%(tex\|none\)\>'
	let b:lag_markup = "unknown"
    endif
endif
...
if b:lag_markup == "unknown"
    if search('\\documentclass\|\\begin{\(code}\)\@!\|\\\(sub\)*section\|\\chapter|\\part','W') != 0
	let b:lag_markup = "tex"
    else
	let b:lag_markup = "plain"
    endif
endif
...
if b:lag_markup == "tex"
    ...
    syntax cluster lagTeXContainer contains=tex.*Zone,texAbstract
else
    syntax cluster lagTeXContainer contains=.*
endif

" Literate Haskell is Haskell in between text, so at least read Haskell
" highlighting
if version < 600
    syntax include @uuagTop <sfile>:p:h/uuag.vim
else
    syntax include @uuagTop syntax/uuag.vim
endif

syntax region lagHaskellBirdTrack start="^>" end="\%(^[^>]\)\@=" contains=@uuagTop,lagBirdTrack containedin=@lagTeXContainer
syntax region lagHaskellBeginEndBlock start="^\\begin{code}\s*$" matchgroup=NONE end="\%(^\\end{code}.*$\)\@=" contains=@uuagTop,beginCodeBegin containedin=@lagTeXContainer

syntax match lagBirdTrack "^>" contained
